...
void execute_pipes() {
	...
	// in childpid
		...
		// pod piper
			...
			commands = malloc(commandCount * sizeof(struct command));
			...
			for(i = 0; i < commandCount; ++i) {
				...
				if(i == 0) { 
					...
					commands[0].argv = malloc(j*sizeof(char*));
					int k = 0;
					// copy arguments over
					for(k = 0; k < j; ++k) {
						commands[0].argv[k] = malloc((strlen(arguments[k]) + 1) * sizeof(char));
						strcpy(commands[0].argv[k], arguments[k]);
					}
					commands[0].argv[j] = (char*) 0;
				}
				// LAST
					...
					commands[commandCount - 1].argv = malloc(j*sizeof(char*));
					int k = 0;
					// copy arguments over
					for(k = 0; k < j; ++k) {
						commands[commandCount - 1].argv[k] = malloc((strlen(arguments[k]) + 1) * sizeof(char));
						strcpy(commands[commandCount - 1].argv[k], arguments[k]);
					}
					commands[commandCount - 1].argv[j] = (char*) 0;
				}
				// MID
				else { 
					printf("mid\n");
					...
					commands[i].argv = malloc(j*sizeof(char*));
					int k = 0;
					for(k = 0; k < j; ++k) {
						commands[i].argv[k] = malloc((strlen(arguments[k]) + 1) * sizeof(char));
						strcpy(commands[i].argv[k], arguments[k]);
					}
					commands[i].argv[j] = (char*) 0;
				}		
			}
			int x;
			int inChannel;
			...
			for (x = 0; x < commandCount - 1; ++x) {
				pipe (fd);
				//spawn_proc (inChannel, fd [1], commands + x); //take in read end of previous iteration, goes to write end of next iteration
				...
				if ((pid = fork ()) == 0) { //in parents
					...
					execvp ((commands + x)->argv [0], (char * const *)(commands + x)->argv);
			    }
				//return pid;
			...
			if(execvp (commands[x].argv [0], (char * const *)commands[x].argv) == -1) { //error
